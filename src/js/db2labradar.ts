...
import { Unit_Velocity,Unit_System,Unit_Distance,Unit_Energy, Unit_Weight } from "./_types";
...
export async function db2Labradar(fileid: number):Promise<File> {
  return new Promise((resolve,reject) => {
    let stream = getLabradartemplate()
    Promise.all([GetFile(fileid), GetUnits(fileid), GetStats(fileid), GetShots(fileid)])
    .then(([filedata,units,stats,shots])=> {
      const filename = generateFilename(filedata.name, filedata.title, stats.timestamp);
      stream = stream.replace("{DEVICEID}", filedata.deviceid ?? "useyournose-xeroconv");
